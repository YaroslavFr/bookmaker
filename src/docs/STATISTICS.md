...
## Главная страница: события и купон

Этот раздел кратко описывает ключевую цепочку формирования данных и рендера главной страницы, чтобы связать статистику со списком матчей и купоном.

### Маршруты
- `GET /` → `App\Http\Controllers\BetController@index` — рендер главной страницы.
- `GET /odds` → `OddsController@odds` — получение коэффициентов.
- `GET /events/{event}/markets` → `OddsController@markets` — дополнительные рынки по событию.
- `GET /odds/game/{gameId}` → `OddsController@marketsByGame` — рынки по внешнему идентификатору игры.

### Формирование событий
- Источник: таблица `events` (поля команд, дата/время, статус, опционально `competition` и `external_id`).
- Исторически наполняется командами синхронизации коэффициентов и результатов (EPL/UCL/ITA).
- Контроллер `index()` собирает ленты лиг EPL/UCL/ITA, карту соответствия `event_id → external_id` и историю купонов.

### Рендер и данные view()
- `view('home', data)` принимает имя шаблона Blade и массив данных.
- Возвращает `Illuminate\View\View`, конвертируемый в HTML.
- На странице доступны переменные: `leagues` (для секций лиг), `eventsEpl/Ucl/Ita`, `coupons`, `marketsMap`, `gameIdsMap`.

### Купон и коэффициенты
- Основная колонка коэффициентов: `home_odds / draw_odds / away_odds`.
- Дополнительные рынки имеют атрибуты `data-market`, `data-selection`, `data-odds`.
- Клик по коэффициенту добавляет исход в купон (Vue‑компонент), купон поддерживает несколько событий одновременно.